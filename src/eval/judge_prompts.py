...
=========================
MMHal-Bench evaluation prompt template for the Qwen2.5-3B-Instruct judge.

The ``TEMPLATE`` string accepts four positional format arguments:

1. ``image_content``  – comma-separated list of image content labels.
2. ``question``       – the original question posed to the LMM.
3. ``gt_answer``      – the standard human-generated reference answer.
4. ``model_answer``   – the LMM response to be evaluated.

The judge is instructed to:
  * Provide a short natural-language explanation first.
  * Then emit a rating on the scale 0–6:

  ======  ===================================================
  Rating  Meaning
  ======  ===================================================
  6       Very informative, good analysis/reasoning, no hallucination
